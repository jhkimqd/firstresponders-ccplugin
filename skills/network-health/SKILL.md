...
1. **`polygon-datadog`** MCP — for monitors, incidents, active alerts, and recent metric anomalies.
2. **`polygon-rpc`** MCP — for chain-level liveness (`get_chain_status`, `get_recent_blocks`, `get_gas_usage`).
...
1. **Start chain-level checks in parallel with Datadog checks:**
   ...
   - Query `polygon-datadog` MCP tools for:
...
2. **Synthesize a concise status report** with the following structure:
   ...
   - **Sources**: list the MCP tools called (e.g., `polygon-datadog.list_monitors`, `polygon-rpc.get_chain_status`).
